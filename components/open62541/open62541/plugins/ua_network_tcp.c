...
# define SOCKET int
# define WIN32_INT
// # include <arpa/inet.h> // ESP_IDF don't have sys/select.h, All its stuff is somewhere else implemented
// # include <netinet/in.h>
// # include <sys/select.h>
// # include <sys/ioctl.h>
# include <fcntl.h>
# include <unistd.h> // read, write, close
# include <sys/unistd.h> // gethostname, (should have "gai_strerror")
# include <netdb.h>

// TODO: I believe I should add sys/socket.h manually here. Because ESP-IDF is not mentioned here! :D
...
#endif
# ifndef __CYGWIN__
//#  include <netinet/tcp.h> // for lwip this can be found in "lwip/include/lwip/lwip/tcp.h"
# endif
#endif
    ...
    do {
        ...
        do {
            ...
            if(n < 0 && errno__ != INTERRUPTED && errno__ != AGAIN) {
                ...
                connection->lwip_close_r(connection);
...
#else
    int opts = fcntl(sockfd, F_GETFL, 1); // This is the hacking solution
...
//#endif
        ...
        if (tcpip_adapter_get_ip_info(ESP_IF_WIFI_STA, &ip) == ESP_OK) { // Get wifi station interface info
            // ESP_LOGI(TAG, "WIFI IP:"IPSTR, IP2STR(&ip.ip));
	    // TODO: clean right now hostname
	    // TODO: Write IP address into hostname
	    // TODO: Pass these discovery into discoveryUrl
	    du.length = (size_t)snprintf(discoveryUrl, 255, "opc.tcp://"IPSTR":%d",IP2STR(&ip.ip),layer->port);
        } else {
            du.length = (size_t)snprintf(discoveryUrl, 255, "ERROR happen when get ip address.");
	}
    ...
    SOCKET newsock = socket(PF_INET, SOCK_STREAM, IPPROTO_TCP); // Take care if I shouldn't use IPPROTO_TCP
#ifdef _WIN32
...
UA_ClientConnectionTCP(UA_ConnectionConfig conf, const char *endpointUrl,
    ...
    if(error != 0 || !server) {
        UA_LOG_WARNING(logger, UA_LOGCATEGORY_NETWORK,
                ...
                hostname, printf("error happend in ua_network_tcp.c line 711")/*gai_strerror(error)*/); // There is still no gai_strerror in esp-idf
        return connection;
    }
